...
def cache_product() -> DataFrame:    
    # categories = st.secrets.mpr.categories.keys()
    # formatted_categories = ', '.join(
    #     [f"'{category}'" for category in categories]
    # )

    # query = FETCH_PRODUCT.format(
    #     product_table=st.secrets.mpr.table.product_table,
...
def _fetch_images(row):
    ...
    try:
        ...
        image_save_path = os.path.join(st.secrets.mpr.product_image_dir, f"{item_id}.jpg")
