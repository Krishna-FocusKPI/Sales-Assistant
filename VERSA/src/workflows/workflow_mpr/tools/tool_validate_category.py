...
class ValidateCategory(BaseTool):
    ...
    def _on_success(self, category: str) -> str:
        ...
        if not workflow:
            return "Sucessfully validate the input category."
...
def category_exist(target: str):
    ...
    # find match
    for category, alternatives in st.secrets.mpr.categories.items():
...
def _infer_category(category: str):
    ...
    categories_explaination = "\n".join([f"{category} - {categories_explaination}" for category, categories_explaination in st.secrets.mpr.categories_explanation.items()])
