...
def category_exist(target: str):
    ...
    # find match
    for category, alternatives in st.secrets.ipr.categories.items():
...
def _infer_category(category: str):
    ...
    categories_explaination = "\n".join([f"{category} - {categories_explaination}" for category, categories_explaination in st.secrets.ipr.categories_explanation.items()])
...
def _to_prompt_bot(instruction, use_service=False):
    ...
    try:
        ...
        logging.warning("Direct LLM fallback for IPR category inference failed: %s", e)
        return "none"