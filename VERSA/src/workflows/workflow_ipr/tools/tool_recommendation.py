...
class ProductRecommendation(BaseTool):
    ...
    def _run(self) -> str:
        ...
        naics_code, category = memory.naics_code, memory.category
        slugger_file =  cache_product()
        slugger_file = slugger_file[slugger_file['CATEGORY'].str.lower() == category.lower()]
        
        try:
            # Step 1. Get the number of recommendations
            logging.info(f"* Pull recommendations for : naics_code-{naics_code}, category-{category}")
            num_of_recommendations = st.secrets.ipr.num_of_recommendations
            
            # Step 2. Get recommendation from database
            all_available_products = _fetch_recommendation_list_from_SF(naics_code, category)
            if all_available_products.empty:
                raise ValueError(f"No recommendations found for {category} category with NAICS code {naics_code}")
            ...
            logging.error(f"Error processing recommendations, param-category-{category}, param-naics_code-{naics_code}", exc_info=True)
...
# DATA HANDLING
# =============================================================================
def _fetch_recommendation_list_from_SF(naics_code, category) -> DataFrame:
    """
    ...
    #             AND
    #             LOWER(naics_code) = LOWER('{naics_code}')
    #         ORDER BY RANK
    #         """).format(
    #             recommendation_table=st.secrets.ipr.table.recommendation_table,
    #             naics_code=naics_code,
    #             category=category
    #         )
    # conn = st.connection("snowflake")
    # return conn.query(query)
    df = load_workflow_csv("ipr_recommendation.csv")
