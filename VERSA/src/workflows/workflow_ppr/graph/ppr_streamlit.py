"""
Wire PPR LangGraph to Streamlit. Graph state = messages only. Tools use
st.session_state.workflow['workflow_memory'] for context (set by init_ppr_workflow).
"""
import logging
...
def run_ppr_turn(user_message: str, *, provider: str = "openai") -> None:
    """
    Run one PPR LangGraph turn. State = messages only. Tools read/write
    st.session_state.workflow['workflow_memory'] (already set by init_ppr_workflow).
    Pass workflow in config so tools can access it when running in a worker thread.
    """
    logging.info("%sUsing provider: %s (PPR)%s", GREEN, provider, RESET)
    session_messages = st.session_state.get("messages") or []
    lc_messages = _session_messages_to_lc(session_messages)
    initial_state: PPRState = {"messages": lc_messages}

    workflow = st.session_state.get("workflow")
    config = {"configurable": {"workflow": workflow}} if workflow else None

    graph = build_ppr_graph(provider=provider)
    ...
    if final_content is not None:
        ...
        if activity and "product_recommendation" in activity:
            ...
            if mem is not None:
                products_chat_embed = build_products_chat_embed_from_memory(mem, variant="ppr")
