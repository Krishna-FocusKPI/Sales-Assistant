...
class AddProduct(BaseTool):
    ...
    def _on_failure(self, message=None):
        workflow = get_workflow()
        if not workflow:
            return message or "Failed to add product."
        to_next = workflow['to_next_memory']
        to_next.reset()
        to_next.decision = "FAILED"
        to_next.source = self.name
...
def _add_top_products(criteria: str, parsed=False): 
    ...
    workflow = get_workflow()
    if not workflow:
        return False, None, "Error: workflow context not available."
    mem = workflow['workflow_memory']
    filtered_products = mem.filtered_products
    all_available_products = mem.all_available_products
...
def _add_products(product_ids: list):
    ...
    mem = workflow['workflow_memory']
    all_available_products = mem.all_available_products
    shopping_cart = mem.shopping_list
    ...
    if products_to_add['FROM_SLUGGER'].any():
        ...
        if slugger_items.shape[0] == 1:
            ...
            message = (
                ...
                f"which means {mem.logo_name} may have already purchased it in the past 6 months."
            )
        else:
            slugger_items = ', '.join(slugger_items['ITEM_ID'].astype(str).tolist())
            ...
            message = (
                ...
                f"which means {mem.logo_name} may have already purchased them in the past 6 months."
            )
