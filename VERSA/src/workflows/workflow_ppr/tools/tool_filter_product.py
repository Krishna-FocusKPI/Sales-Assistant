...
from src.common.provider import get_chat_model, get_default_provider, get_embeddings
...
class FilterProduct(BaseTool):
        ...
        while retries < 2:
            try:
                ...
                if last_round_products is None or last_round_products.empty or filter_update_status == "update":
                    last_round_products = memory.all_available_products
    ...
    def _on_success(self, filters, filtered_products):
        workflow = get_workflow()
        if not workflow:
            return "Sucessfully get the filtered products."
        memory = workflow['workflow_memory']
    ...
    def _on_failure(self, message=None):
        workflow = get_workflow()
        if not workflow:
            return message or "Failed to execute your filtering condition."
        to_next = workflow['to_next_memory']
        to_next.reset()
...
def _update_filter_status(last_round_filter, criteria):
    ...
    sql_columns = st.secrets.ppr.product.sql_filterable_columns
    rag_columns = st.secrets.ppr.product.rag_filterable_columns
...
def _to_prompt_bot(instruction, use_service=False):
    ...
    # Worker thread: call LLM directly
    try:
