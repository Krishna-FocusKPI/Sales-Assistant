...
class EndShopping(BaseTool):
    ...
    def _run(self, decision: str) -> str:
        ...
        try:
            ...
            if shopping_list is None or memory.shopping_list.shape[0] == 0:
    ...
    def _on_success(self) -> str:
        workflow = get_workflow()
        if not workflow:
            return "Sucessfully ended the shopping process."
    ...
    def _on_failure(self, decision=None) -> str:
        workflow = get_workflow()
        if not workflow:
            return decision if decision else "The input for end the shopping process is invalid."
        to_next = workflow['to_next_memory']
        to_next.reset()
    ...
    def _on_error(self) -> str:
        workflow = get_workflow()
        if not workflow:
            return "An error occurred while ending the shopping process."
        to_next = workflow['to_next_memory']
        to_next.reset()
