...
class ResetFilter(BaseTool):
    ...
    def _run(self, reset_string: str) -> str:
        ...
        try:
            if reset_string == 'none':
                ...
                del filters[name]
                all_available_products = memory.all_available_products
    ...
    def _on_success(self, filters, filtered_products: str, message=None) -> str:
        workflow = get_workflow()
        if not workflow:
            return message or "Sucessfully reset filters."
        memory = workflow['workflow_memory']
    ...
    def _on_failure(self, reset_string) -> str:
        workflow = get_workflow()
        if not workflow:
            return "The filter could not be reset."
        to_next = workflow['to_next_memory']
        to_next.reset()
    ...
    def _on_error(self, reset_string: str) -> str:
        workflow = get_workflow()
        if not workflow:
            return f"An error occurred while reset filter: {reset_string}"
        to_next = workflow['to_next_memory']
        to_next.reset()
...
def _to_prompt_bot(instruction, use_service=False):
    ...
    try:
        ...
        logging.warning("Direct LLM fallback in reset_filter failed: %s", e)
        return ""
